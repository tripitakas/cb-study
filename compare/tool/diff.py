...
class Diff(object):
    ...
    def pre_base(cls, base, keep_line=True):
        ...
        return base  # re.sub(Diff.base_junk_char, '', base)

    @classmethod
    def pre_cmp(cls, cmp):
        """ 比对本预处理，过滤换行符以及非中文字符"""
        return cmp  # re.sub(Diff.cmp_junk_char, '', cmp)

    @classmethod
    def diff(cls, base='', cmp1='', cmp2='', cmp3='', label=None):
        ...
        if cmp1:
            ret1 = diff_func(base, cmp1, {'base': lbl['base'], 'cmp': lbl['cmp1']})
            ret, _err = Diff._merge_by_combine(ret, ret1, base_key=lbl['base'])
            err.extend(_err)
        if cmp2:
            ret2 = diff_func(base, cmp2, {'base': lbl['base'], 'cmp': lbl['cmp2']})
            ret, _err = Diff._merge_by_combine(ret, ret2, base_key=lbl['base'])
            err.extend(_err)
        if cmp3:
            ret3 = diff_func(base, cmp3, {'base': lbl['base'], 'cmp': lbl['cmp3']})
    ...
    @classmethod
    def _diff_two_v1(cls, base, cmp, label=None):
        lbl = {'base': 'base', 'cmp': 'cmp'}
        if label and isinstance(label, dict):
            lbl.update(label)
